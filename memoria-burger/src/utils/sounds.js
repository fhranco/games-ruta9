...
class SoundEngine {
    ...
    // 🚨 Sonido específico para ingrediente equivocado (Buzzer corto + voz correcta)
    playWrong() {
        this.init();
        if (!this.ctx) return;
        try {
            ...
            // Buzzer de error corto y contundente (tres pulsos rápidos descendentes)
            [0, 0.08, 0.16].forEach((offset, i) => {
                ...
                osc.frequency.exponentialRampToValueAtTime(freq * 0.6, now + offset + 0.07);
            ...
            // Voz correcta y específica para el error de ingrediente
            setTimeout(() => {
                this.announce('¡Ingrediente incorrecto! Intenta de nuevo.');
            }, 120);
        } catch (e) {
            console.warn('Fallo en playWrong():', e);
        }
    }

    playWin() {
        this.playGameResult('perfect');
    }

    // playLose() solo se llama cuando se acaba el tiempo (game over)
    playLose() {
        this.init();
        if (!this.ctx) return;
        try {
