...
function App() {
  ...
  const handleStop = async (matchedPairsCount) => {
    ...
    try {
      ...
      const gameStock = localDb.gameStock["memoria-burger"];

      // Dependiendo de las parejas encontradas, mejores premios
      let allowedPrizes = ["HELADO_SOFT"];
      if (matchedPairsCount >= 7) {
        allowedPrizes = ["DESCUENTO_20", "REGALO_SORPRESA", "SCHOP_BEBIDA"];
      } else if (matchedPairsCount >= 4) {
        allowedPrizes = ["PAPAS_FRITAS", "DESCUENTO_10"];
      }
      ...
      if (possiblePrizes.length > 0) {
        ...
        // Descontar stock
        localDb.gameStock["memoria-burger"][selectedPrizeId]--;
